  ...
  return (
    <div className="relative rounded-2xl p-5 h-full flex flex-col bg-white/10 backdrop-blur-2xl border border-white/25">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin-white space-y-4 pr-1">
        <div>
          <h3 className="text-white font-black text-xl leading-snug pr-8">
            {metier.label}
          </h3>

          <div className="flex flex-wrap gap-2 mt-2">
            <span className="text-white text-xs font-bold bg-white/20 px-2.5 py-1 rounded-md">
              {Array.isArray(metier.domaine)
                ? metier.domaine.join(", ")
                : metier.domaine}
            </span>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {metier.parcours.map((p, i) => (
                <span
                  key={i}
                  className="text-[11px] bg-white/15 border border-white/20 px-2.5 py-1 rounded-full text-white"
                >
                  {p}
                </span>
              ))}
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {metier.profil.map((p, i) => (
                <span
                  key={i}
                  className="text-[11px] bg-white/20 border border-white/30 text-white px-2.5 py-1 rounded-full"
                >
                  {p}
                </span>
              ))}
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-1">
        <span className="text-white font-bold text-base leading-snug flex-1">
          {metier.label}
        </span>

        <span className="text-white/90 text-[10px] font-bold bg-[#1a3ea8]/80 px-2 py-0.5 rounded-full shrink-0">
          {Array.isArray(metier.niveau)
            ? metier.niveau.join(", ")
            : metier.niveau}
        </span>
      ...
      {metier.description && (
        <p className="text-white/75 text-xs leading-relaxed line-clamp-2 mb-1.5">
          {metier.description}
        </p>
      )}
...
export default function Section2({
  ...
  const domainesList = useMemo(
    () =>
      allDomaines.map((d) => ({
        id: d.id,
        label: d.label,
        keywords: [String(d.label || "").toLowerCase()],
      })),
    [allDomaines]
  );
  ...
  const filteredMetiers = useMemo(() => {
    ...
    return allMetiers.filter((m) => {
      const domaineStr = Array.isArray(m.domaine)
        ? m.domaine.join(" ")
        : m.domaine || "";
      ...
      const searchText = [
        ...
        domaineStr,
        m.description || "",
        Array.isArray(m.parcours) ? m.parcours.join(" ") : "",
      ]
        .join(" ")
        .toLowerCase();
    ...
    return allMetiers.filter((m) => {
      if (Array.isArray(m.domaine)) {
        return m.domaine.some((d) => {
          ...
          return (
            dNorm === domaineNorm ||
            dNorm.includes(domaineNorm) ||
            domaineNorm.includes(dNorm)
          );
        });
      }

      const fieldDomNorm = String(m.domaine || "").toLowerCase().trim();

      return (
        fieldDomNorm === domaineNorm ||
        fieldDomNorm.includes(domaineNorm) ||
        domaineNorm.includes(fieldDomNorm)
      );
    });
  ...
  useEffect(() => {
    const handler = (e) => {
      if (
        metierComboRef.current &&
        !metierComboRef.current.contains(e.target)
      ) {
        setIsMetierComboOpen(false);
      }
  ...
  return (
    ...
    >
        ...
        <div className="fixed inset-0 z-[500] bg-gradient-to-br from-[#1250c8] via-[#1a6dcc] via-[#28b090] via-[#a0d820] to-[#c2e832] flex items-center justify-center">
          <div className="s2-deco-tr">
            <img
              src={pictoExplorer}
              alt=""
              className="w-[160px] lg:w-[260px] opacity-40 object-contain"
            />
          </div>
          ...
          <div className="relative z-10 flex flex-col items-center gap-8">
            ...
            <div className="mt-20 flex flex-col items-center gap-2">
              ...
              <p className="text-white/60 text-sm font-medium">
                Préparation de la fiche métier...
              </p>
            </div>
          </div>
        </div>
      ...
      <div
        className={`s2-layout ${metierFocusLayout ? "metier-focus" : ""} ${
          metierFocusLayout && isMetierComboOpen ? "combo-open" : ""
        }`}
      >
        <div
          className={`s2-left transition-transform duration-500 ease-in-out ${
            metierFocusLayout
              ? "translate-y-0"
              : isAnyComboOpen
              ? "lg:-translate-y-20 -translate-y-12"
              : "translate-y-0"
          }`}
        >
          <div className="s2-scroll">
            ...
            <div
              className={`s2-header-mob transition-all duration-500 ease-in-out ${
                isAnyComboOpen ? "opacity-30 scale-95" : "opacity-100 scale-100"
              }`}
              style={{
                animation:
                  "s2In 0.55s cubic-bezier(0.16,1,0.3,1) 0.15s both",
              }}
            >
              <h1 className="s2-h1">EXPLORER LES MÉTIERS</h1>

              <p className="s2-desc">
                Aide les élèves et les parents à choisir un métier et le
                parcours d'études adapté.
              </p>
            </div>

            {!isComboOpen && (
              <div
                className={`s2-cbwrap ${isMetierComboOpen ? "open" : ""}`}
                ref={metierComboRef}
              >
                ...
                {isMetierComboOpen && (
                  <div className="s2-drop-new s2-fadein">
                    ...
                    <div
                      id="metier-options"
                      className="s2-drop-list-new"
                      role="listbox"
                      aria-label="Liste des metiers"
                    >
                      {filteredMetiers.length > 0 ? (
                        filteredMetiers.map((m) => (
                          <button
                            ...
                            className={`s2-drop-item-new ${
                              localSelected?.id === m.id ? "active" : ""
                            }`}
                            role="option"
                          ...
                          >
                            <div className="flex flex-col">
                              ...
                              <span className="s2-item-badge">
                                {Array.isArray(m.domaine)
                                  ? m.domaine.join(", ")
                                  : m.domaine}
                              </span>
                            </div>

                            {localSelected?.id === m.id && (
                              <HiCheck className="text-blue-600" size={14} />
                            )}
                          </button>
                        ))
                      ) : (
            ...
            {mode !== "metier" &&
              ...
              !isMetierComboOpen && (
                <div
                  ...
                  style={{
                    animation:
                      "s2In 0.55s cubic-bezier(0.16,1,0.3,1) 0.35s both",
                  }}
                >
                  <div className="s2-sep-l" />
                  <span className="s2-sep-txt">ou</span>
                  <div className="s2-sep-l" />
                </div>
              )}
            ...
            {mode !== "metier" && !focusMetierSearch && (
              <>
                <div
                  className={`s2-cbwrap ${isComboOpen ? "open" : ""} ${
                    isMetierComboOpen ? "hidden" : ""
                  }`}
                  ref={comboRef}
                >
                  ...
                  >
                    <span
                      className={selectedDomaine ? "s2-val-new green" : "s2-ph"}
                    >
                      {selectedDomaine
                        ...
                        {filteredDomaines.length > 0 ? (
                          filteredDomaines.map((d) => (
                            <button
                              ...
                              className={`s2-drop-item-new ${
                                selectedDomaine?.id === d.id ? "active" : ""
                              }`}
                              role="option"
                ...
                {mode === "domaine" && selectedDomaine && (
                  <div className="s2-dom-res s2-fadein lg:hidden">
                    <h2 className="s2-dom-cnt">
                      {metiersParDomaine.length} métier
                      {metiersParDomaine.length > 1 ? "s" : ""}
                    </h2>
                      ...
                      <div className="s2-cards">
                        {metiersParDomaine.map((m) => (
                          <MetierCard
                            key={m.id}
                            metier={m}
                            onSelect={(m) => handleSelectMetier(m, true)}
                          />
                        ))}
                      </div>
                    ) : (
                      <p className="s2-nores">
                        Aucun métier trouvé pour ce domaine.
                      </p>
                    )}
                  </div>
                )}
        ...
        <div
          className={`s2-right ${
            metierFocusLayout ? "metier-focus-hidden" : ""
          }`}
        >
          {mode === "domaine" && selectedDomaine && (
            <div className="s2-r-dom s2-fadein">
              <div className="s2-r-domhdr">
                <h2 className="s2-dom-cnt lg">
                  {metiersParDomaine.length}
                  <span className="sub">
                    {" "}
                    métier{metiersParDomaine.length > 1 ? "s" : ""}
                  </span>
                </h2>
      ...
      {mode === "metier" && (
        <div
          className={`fixed bottom-20 left-1/2 -translate-x-1/2 z-[90] w-full max-w-sm px-6 pointer-events-none flex justify-center transition-all duration-300 ${
            isMetierComboOpen
      ...
      <style>{`
        ...
        .s2-layout.metier-focus.combo-open {
          z-index: 9990;
          overflow: visible;
        }
        ...
        .s2-layout.metier-focus .s2-left {
          width: 100%;
          align-items: center;
        }
        ...
        .s2-layout.metier-focus.combo-open .s2-scroll {
        ...
        .s2-drop-item-new:hover {
          background: #eff6ff;
        }

        .s2-drop-item-new.active {
          background: #eff6ff;
          color: #2563eb;
        }
        ...
        .s2-r-list::-webkit-scrollbar {
          width: 5px;
        }

        .s2-r-list::-webkit-scrollbar-track {
          background: transparent;
        }

        .s2-r-list::-webkit-scrollbar-thumb {
        ...
        .s2-fadein {
          animation: s2FadeIn 0.3s cubic-bezier(0.16, 1, 0.3, 1) both;
        }
